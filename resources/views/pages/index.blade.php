...
@section('content')
    ...
    <section x-data="{ 
        ...
        <div class="relative h-screen w-full overflow-hidden bg-black">
            ...
            <div class="relative h-full w-full">
                <template x-for="(slide, index) in slides" :key="index">
                    <div x-show="activeSlide === index" 
                        ...
                        <div class="absolute inset-0 z-10 bg-linear-to-r from-black/80 via-black/40 to-transparent"></div>
            ...
            {{-- Slider Controls --}}
            <button @click="prev()" class="hidden lg:block absolute left-8 top-1/2 z-30 -translate-y-1/2 text-white/50 hover:text-white">
                <svg class="h-12 w-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </button>
            <button @click="next()" class="hidden lg:block absolute right-8 top-1/2 z-30 -translate-y-1/2 text-white/50 hover:text-white">
            ...
            <div class="absolute bottom-10 left-1/2 z-30 flex -translate-x-1/2 space-x-3">
                <template x-for="(slide, index) in slides" :key="index">
                    <button @click="activeSlide = index" 
                        :class="activeSlide === index ? 'bg-[#0A81CB] w-8' : 'bg-white/50 w-3'"
        ...
        <div class="mx-auto max-w-[80%] lg:max-w-[70%]">
            ...
            <div class="services-grid mt-[4rem] lg:max-w-[1200px] mx-auto grid grid-cols-1 gap-[2rem] md:grid-cols-2 lg:grid-cols-3">
                @if(isset($services) && count($services) > 0)
                    ...
                    <div class="service-card flex flex-col items-center rounded-[1rem] bg-white p-[2.5rem] text-center shadow-[0_10px_50px_rgba(0,0,0,0.05)] transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_20px_60px_rgba(0,0,0,0.1)]">
    ...
    {{-- recent post --}}
    <section class="bg-white py-8 lg:py-24 overflow-hidden" id="blog-section">
        <div class="container mx-auto px-6 lg:px-8">
